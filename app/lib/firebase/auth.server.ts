...
export const signUp = async (name: string, email: string, password: string) => {
  await auth.server
    ...
    .then((userRecord) => {
      ...
      const lastName = userRecord.displayName?.split(' ')[1];
      if (userRecord.uid && firstName && lastName) {
        addUser(userRecord.uid, firstName, lastName);
      }
    })
    .catch((error) => {
      console.log('Error caught creating new user', error);
    });
